...
class ThemeCatalogTest {
    ...
    fun `catalog exposes all existing designs plus the Alaa Neon IPTV choice`() {
        assertEquals(26, ThemeCatalog.options.size)
        assertEquals(26, ThemeCatalog.options.map { it.id }.toSet().size)
    ...
    fun `later tv themes expose distinct structural layouts not only palette changes`() {
        ...
        assertEquals(15, newThemeIds.size)
        ...
        assertEquals(15, signatures.toSet().size)
