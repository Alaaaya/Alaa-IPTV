...
object DisplayTheme {
    ...
    fun applyDashboard(binding: ActivityDashboardBinding, prefs: AppPreferences) {
        ...
        val usesCinematicShell = isNeonIptv || isFigmaAlaa || isAsinat || isAsinat2 || isAya || isAya2
        val density = binding.root.resources.displayMetrics.density
        binding.root.setBackgroundColor(Color.parseColor(palette.background))
        binding.sidebarContainer.background = if (isNeonIptv || isAsinat || isAsinat2 || isAya || isAya2) {
        ...
        binding.heroWatchNow.background = rounded(
            ...
            if (isNeonIptv) "#FF8994" else if (isAsinat || isAsinat2 || isAya || isAya2) palette.focusStroke else null,
            if (isNeonIptv || isAsinat || isAsinat2 || isAya || isAya2) 1 else 0
        )
        ...
        if (usesCinematicShell) {
            ...
            (binding.topSearchGroup.layoutParams as? RelativeLayout.LayoutParams)?.let { params ->
                ...
                params.width = ((if (isAya2) 296 else if (isAya) 274 else if (isAsinat2) 324 else if (isAsinat) 310 else 290) * density).toInt()
            ...
            binding.heroCard.radius = (if (isAya2) 18f else if (isAya) 16f else if (isAsinat2) 10f else if (isAsinat) 14f else 22f) * density
            binding.heroCard.cardElevation = if (isAya2) 8f else if (isAya) 5f else if (isAsinat2) 4f else if (isAsinat) 6f else 10f
    ...
    fun applyLive(binding: ActivityMainBinding, prefs: AppPreferences) {
        ...
        val isAya = isAya(prefs.displayTheme)
        val isAya2 = isAya2(prefs.displayTheme)
        val quietLiveSurfaces = simpleLive || isAsinat || isAsinat2 || isAya || isAya2
    ...
    fun applyMovies(binding: ActivityMoviesBinding, prefs: AppPreferences) {
        ...
        if (isAsinat(prefs.displayTheme) || isAsinat2(prefs.displayTheme) || isAya(prefs.displayTheme) || isAya2(prefs.displayTheme)) {
            binding.sidebarContainer.layoutParams = binding.sidebarContainer.layoutParams.apply {
                width = ((when {
            ...
            binding.movieCategoryPanel.layoutParams = binding.movieCategoryPanel.layoutParams.apply {
                width = ((if (isAya2(prefs.displayTheme)) Aya2LayoutPolicy.MOVIE_CATEGORY_WIDTH_DP else if (isAya(prefs.displayTheme)) AyaLayoutPolicy.MOVIE_CATEGORY_WIDTH_DP else if (isAsinat2(prefs.displayTheme)) 260 else 280) * density).toInt()
            }
            val categoryPadding = if (isAya2(prefs.displayTheme)) 16 else 14
            ...
            binding.moviesTitle.setTextColor(Color.parseColor(if (isAya2(prefs.displayTheme)) "#FFF0F5" else "#F5F7FA"))
    ...
    fun applySeries(binding: ActivitySeriesBinding, prefs: AppPreferences) {
        ...
        if (isAsinat(prefs.displayTheme) || isAsinat2(prefs.displayTheme) || isAya(prefs.displayTheme) || isAya2(prefs.displayTheme)) {
            ...
            binding.seriesTitle.setTextColor(Color.parseColor(if (isAya2(prefs.displayTheme)) "#FFF0F5" else "#F5F7FA"))
    ...
    fun applySeriesDetails(binding: ActivitySeriesDetailsBinding, prefs: AppPreferences) {
        ...
        if (isAsinat(prefs.displayTheme) || isAsinat2(prefs.displayTheme) || isAya(prefs.displayTheme) || isAya2(prefs.displayTheme)) {
            binding.backButton.background = rounded(palette.panel, if (isAya2(prefs.displayTheme)) 16f else 14f, palette.focusStroke, 1)
        }
    }
    ...
    fun applyPlayer(binding: ActivityPlayerBinding, prefs: AppPreferences) {
        ...
        if (isAsinat(prefs.displayTheme) || isAsinat2(prefs.displayTheme) || isAya(prefs.displayTheme) || isAya2(prefs.displayTheme)) {
            binding.channelNameText.background = rounded(palette.sidebar, if (isAya2(prefs.displayTheme)) 16f else 14f, palette.panelStroke, 1)
            binding.playerStatusOverlay.background = rounded(palette.sidebar, if (isAya2(prefs.displayTheme)) 18f else 16f, palette.panelStroke, 1)
            binding.trackSelectionButton.background = rounded(palette.accent, if (isAya2(prefs.displayTheme)) 16f else 14f, palette.focusStroke, 1)
